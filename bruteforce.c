...
void decrypt_with_key(const uint8_t *ciphertext, uint8_t *plaintext, const DES_cblock *key, const DES_cblock *iv) {
    ...
    DES_cbc_encrypt(ciphertext, plaintext, DES_KEY_SIZE, &schedule, &iv_copy, DES_DECRYPT);
}

// Function to encrypt using OpenSSL DES in CBC mode
void encrypt_with_key(const uint8_t *plaintext, uint8_t *ciphertext, const DES_cblock *key, const DES_cblock *iv) {
    DES_key_schedule schedule;
    DES_set_key(key, &schedule);  // Set up the key schedule
    ...
    DES_cbc_encrypt(plaintext, ciphertext, DES_KEY_SIZE, &schedule, &iv_copy, DES_ENCRYPT);
}
...
int tryKey(unsigned long long key, const uint8_t *ciphertext, int len, const char *keyword, const DES_cblock *iv, int id) {
    ...
    return strstr((char *)decrypted, keyword) != NULL;
}
...
int main(int argc, char *argv[]) {
    ...
    const uint8_t correct_plaintext[DES_KEY_SIZE] = {0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x57, 0x72, 0x6C};  // Example: "HelloWrl"
    ...
    if (id == 0) {
        ...
        encrypt_with_key(correct_plaintext, ciphertext, &generated_key, &iv);
